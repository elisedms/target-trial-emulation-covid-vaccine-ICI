...
# OUTCOME : Overall survival or progression-free survival
#######################################
...
df_restricted = df %>% 
  ...
  #Since we do not have information on the vaccine date -- we impute the date of vaccine at the end of the grace period
  mutate(imputed_vaccine_delay = 100/30.436875)
...
surv = estimate_survival(df_restricted_clone_long, 
                  ...
                  ylab_txt = ylab_txt,
                  print = TRUE,
                  title_txt = "After cloning, censoring\nand weighting",
...
#####################################################
# Estimate marginal hazard ratios (not presented in the article)
####################################################

#Change order of factor for the hazard ration of vaccine versus no vaccine
df_restricted_clone_long_hr = df_restricted_clone_long %>%
  mutate(clone = factor(clone, levels = rev(label_order)))

model_hr = coxph(Surv(tstart, tstop, status_os) ~ clone,
                 data = df_restricted_clone_long_hr, weights = df_restricted_clone_long_hr$weight)
hr_estimate = summary(model_hr)$coefficients[1,"exp(coef)"]
...
for (bootstrap_number in 1:number_bootstrap){
  ...
  #Change order of factor for the hazard ration of vaccine versus no vaccine
  df_bootstrap_clone_long_hr = df_bootstrap_clone_long %>%
    mutate(clone = factor(clone, levels = rev(label_order)))
  
  #Hazard ratio
  model_hr = coxph(Surv(tstart, tstop, status_os) ~ clone,
                   data = df_bootstrap_clone_long_hr, weights = df_bootstrap_clone_long_hr$weight)
  hr_estimate_bootstrap = summary(model_hr)$coefficients[1,"exp(coef)"]
  hr_estimates_bootstrap = c(hr_estimates_bootstrap,hr_estimate_bootstrap)
...
list_ci = estimate_ci(surv_point_estimates, surv_estimates_bootstrap,
                      outcome = "PFS")
hr_ci = estimate_ci_hr(hr_estimate, hr_estimates_bootstrap)
...
##Print results
print(paste0("melanoma cohort - ",outcome))
print(label_ci_all)
print(paste0("Difference : \n", label_ci_diff))
print("Hazard ratio estimate")
print(hr_ci)
...
plot_survival_curves_align <- cowplot::align_plots(plot_survival_curves,
                                                   ...
                                                   align = 'v', axis ='l')
plot_survival_curves_melanoma <- cowplot::plot_grid(plot_survival_curves_align[[1]],
                                                 plot_survival_curves_align[[2]],
                                               ncol = 1, rel_heights = c(6,2))

print(plot_survival_curves_melanoma)
