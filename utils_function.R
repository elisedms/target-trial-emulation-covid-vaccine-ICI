...
clone_censor <- function(df_restricted,
  ...
  df_restricted_clone = 
    ...
    mutate(status_os = case_when(
      ...
      clone == "No vaccine" & group == "Observed vaccine" & delay_os > imputed_vaccine_delay  ~ 0,
    ...
    mutate(delay_os = case_when(
      ...
      clone == "No vaccine" & group == "Observed vaccine" & delay_os > imputed_vaccine_delay ~ imputed_vaccine_delay,
      TRUE ~ delay_os
    )) %>%
...
#########################################################################
# Estimate 95% confidence intervals from bootstrap samples for hazard ratio
##########################################################################

estimate_ci_hr <- function(hr_estimate, hr_estimates_bootstrap){
  
  ########
  #List of parameters :
  #hr_estimate : point estimate of hazard ratio
  #hr_estimates_bootstrap :list of boostrapped hazard ratios
  ########
  
  #
  hr_ci_low = unname(quantile(hr_estimates_bootstrap,probs = c(0.025)))[1]
  hr_ci_high = unname(quantile(hr_estimates_bootstrap,probs = c(0.975)))[1]
  hr_ci = paste0(round(hr_estimate,2),
                   ", 95%CI (",
                   round(hr_ci_low,2),
                   " - ",
                   round(hr_ci_high,2),
